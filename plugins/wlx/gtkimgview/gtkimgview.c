...
static void tb_hflip_clicked(GtkToolItem *tooleditcopy, GtkWidget *imgview)
{
	...
	gtk_image_view_set_pixbuf(GTK_IMAGE_VIEW(imgview),
							  gdk_pixbuf_flip(gtk_image_view_get_pixbuf(GTK_IMAGE_VIEW(imgview)),
											  TRUE), TRUE);
}
...
HWND DCPCALL ListLoad(HWND ParentWin, char* FileToLoad, int ShowFlags)
{
	...
	g_signal_connect(G_OBJECT(tb_zoom_in), "clicked", G_CALLBACK(tb_zoom_in_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	g_signal_connect(G_OBJECT(tb_zoom_out), "clicked", G_CALLBACK(tb_zoom_out_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	g_signal_connect(G_OBJECT(tb_orgsize), "clicked", G_CALLBACK(tb_orgsize_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	g_signal_connect(G_OBJECT(tb_fit), "clicked", G_CALLBACK(tb_fit_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	g_signal_connect(G_OBJECT(tb_copy), "clicked", G_CALLBACK(tb_copy_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	g_signal_connect(G_OBJECT(tb_rotare), "clicked", G_CALLBACK(tb_rotare_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	g_signal_connect(G_OBJECT(tb_rotare1), "clicked", G_CALLBACK(tb_rotare1_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	g_signal_connect(G_OBJECT(tb_hflip), "clicked", G_CALLBACK(tb_hflip_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	g_signal_connect(G_OBJECT(tb_vflip), "clicked", G_CALLBACK(tb_vflip_clicked),
					 (gpointer)(GtkWidget*)(view));
	...
	if (anim && !gdk_pixbuf_animation_is_static_image(anim))
	{
		...
		g_signal_connect(G_OBJECT(tb_play), "clicked", G_CALLBACK(tb_play_clicked),
						 (gpointer)(GtkWidget*)(view));
		...
		g_signal_connect(G_OBJECT(tb_stop), "clicked", G_CALLBACK(tb_stop_clicked),
						 (gpointer)(GtkWidget*)(view));
...
{
	GdkPixbuf *pixbuf = gtk_image_view_get_pixbuf(GTK_IMAGE_VIEW(find_child(ListWin,
																			"imageview")));
...
int DCPCALL ListSendCommand(HWND ListWin, int Command, int Parameter)
{
	...
	case lc_copy:
		gtk_clipboard_set_image(gtk_clipboard_get(GDK_SELECTION_CLIPBOARD),
								gtk_image_view_get_pixbuf(GTK_IMAGE_VIEW(find_child(ListWin,
																					"imageview"))));
		break;
	default:
